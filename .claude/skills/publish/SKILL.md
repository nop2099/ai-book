...
## Pre-build guard: check for bad edits

**CRITICAL:** The build script wipes `reference/site/` and rebuilds it. Any file edited directly in `reference/site/` will be lost. Before building, check if any tracked or modified files in `reference/site/` are not also present in `reference/_meta/static/` or generated by the build from markdown sources.

```bash
# Check for files in reference/site/ that were modified but don't exist in _meta/static/ or as built markdown
for f in $(git diff --name-only -- reference/site/ 2>/dev/null); do
  base=$(basename "$f")
  if [ ! -f "reference/_meta/static/$base" ] && [ ! -f "reference/$(echo $base | sed 's/.html$/.md/')" ]; then
    echo "WARNING: $f was edited in site/ but has no source in _meta/static/ or markdown. This edit WILL BE LOST on build."
  fi
done
```

If any warnings appear, move the edits to `reference/_meta/static/` before building. Never edit `reference/site/` directly — it is a build output directory.
